...
class YicLogin:
  def __init__(self, root, connection, Error=None):
    ...
    label1 = tk.Label(frame1, text='ユーザー名')
    label1.grid(row=0, column=0)
      
    label2 = tk.Label(frame1, text='パスワード')
    ...
    button_login = tk.Button(
      frame2, text = 'ログイン',
      command = lambda:self.Certification(root, connection, UserName.get(), Password.get()))
    # commandはlambdaを使わないとボタンを押さなくてもcommandが発動されエラーが起こる
  ...
  def Certification(self, root, connection, UserName, Password):
    try:
...
class Register:
  def __init__(self, root, connection, Error=None):
    ...
    frame1 = tk.Frame(root)
    frame1.grid()
    label = tk.Label(frame1, text='登録する情報を入力してください。')
    ...
    button = tk.Button(
      frame2, text='登録',
      command=lambda:self.NewRegister(root, connection, UserName.get(), Password.get()))
    button.grid(row=1, column=0)
    if Error is not None:
      label_Error = tk.Label(frame2, text=Error)
      label_Error.grid(row=2, column=0)

  def NewRegister(self, root, connection, UserName, Password):
      ...
      Register(root, connection, Error)
        ...
        with connection.cursor() as cursor:
          ...
          if results is None:
            ...
            Error = '既に登録されています。名前またはパスワードを変更してください'
            root.destroy()
            root = tk.Tk()
            Register(root, connection, Error)
...
class YicDiary:
  def __init__(self, root, connection,  userID):
    ...
    label_LoginName = tk.Label(rightFrame, text=f'{results["UserName"]}がログイン中')
    label_LoginName.grid(row=3, column=0)
  ...
  def done(self):
    ...
    try:
      ...
      with self.connection.cursor() as cursor:
        ...
        # SQLの作成、定義
        sql = "insert into schedule(User_ID, Kinds_ID, contents) value (%s, %s, %s)"
        # SQLの実行
        cursor.execute(sql, (self.UserID, Kinds_ID, Contents))
        '''
        # 実行結果の受け取り（複数行） 
        results = cursor.fetchall()
        ...
        '''
        # SQLの作成、定義
        sql1 = "select Max(Schedule_ID) from schedule"
        cursor.execute(sql1)
        for i in cursor.fetchone().values():
          MaxSchedule_ID = i
        
        # SQLの作成、定義
        sql2 = "insert into scheduletable(Date, Schedule_ID) value(%s, %s)"
        cursor.execute(sql2, (days, MaxSchedule_ID))
        # 実行結果の受け取り（複数行） 
        results = cursor.fetchall()
        for i, row in enumerate(results):
          print(i, row)
  ...
  def click(self, event):
    ...
    try:
      with self.connection.cursor() as cursor:
        ...
        cursor = self.connection.cursor()
        # トランザクション開始
        sql = "select UserName, Kind, Contents\
              from scheduletable\
              inner join schedule\
              on scheduletable.Schedule_ID = schedule.Schedule_ID\
              inner join Kinds\
              on schedule.Kinds_ID = Kinds.Kinds_ID\
              inner join Users\
        ...
        for j in results:
            LabelText.append(f"(ユーザー){j['UserName']} ： (種別){j['Kind']}  (内容){j['Contents']}")
        if LabelText == []:
